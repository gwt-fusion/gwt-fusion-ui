...
- `gwt-fusion-query-ui`: Optional `QueryView` lifecycle rendering helpers under `org.gwtfusion.query.ui`.
...
- Query UI: optional `QueryView` with configurable idle, loading, error, empty, and success renderers.
...
- Current working status: Milestone 17.2 Query UI Helpers is complete.
